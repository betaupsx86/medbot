...
class TrackerBaseController():
    def __init__(self):
        ...
        self.rate = rospy.get_param('~base_controller_rate', 10)
	self.period=1.0/self.rate  
        ...
        self.goal_distance = rospy.get_param('~goal_distance', 1.1)
	...
	self.move_base = actionlib.SimpleActionClient("move_base", MoveBaseAction)
        ...
        while not rospy.is_shutdown():        
            ...
            elif self.tracker_command == 'BASE_FOLLOW':
                self.base_follow()   
                
            elif self.tracker_command == 'BASE_FOLLOW_NAV':
                self.base_follow_nav() 
    ...
    def base_follow(self):
	drive_vector = self.skeleton['position']['torso']
	#Kp=2.0
	#Ki=1.0
	#following_distance=1.1
	#Kw=1.0

	#	e_d = sqrt(drive_vector.x()**2+drive_vector.y()**2) - self.goal_distance
	#print (e_d)
	#e_w= atan2(drive_vector.y(),drive_vector.x())
	#print (e_w)
	#P=Kp*e_d
	#I=Ki*e_d*(self.period)
	#W= Kw*e_w;

	#if abs(e_d) >1e-2:
	#    self.cmd_vel.linear.x=e_d
	#    self.cmd_vel.angular.z=e_w

	#else:
	#    self.cmd_vel.linear.x=0.0
	#    self.cmd_vel.angular.z=0.0	

	drive_speed = sqrt(drive_vector.x()**2+drive_vector.y()**2) - self.goal_distance
	drive_angle = atan2(drive_vector.y(),drive_vector.x())
    ...
    def base_follow_nav(self):
	print ("here mofo")
	drive_vector = self.skeleton['position']['torso']
	goal = MoveBaseGoal()
	goal.target_pose.header.frame_id = 'base_footprint'
	goal.target_pose.header.stamp = rospy.Time.now()
	goal.target_pose.pose.position.x = drive_vector.x();
	goal.target_pose.pose.position.y = drive_vector.y();
	goal.target_pose.pose.orientation = quaternion_from_euler(0,0,atan2(drive_vector.y(),drive_vector.x()))	
