	...
	class SkeletonTracker
  {
    public:
      ...
      void init()
      {
	    ...
	    n.param("fixed_frame", fixed_frame, std::string("kinect_camera_depth_frame"));
      ...
      void publishTransform(KinectController &kinect_controller, XnUserID const &user, XnSkeletonJoint const &joint, string const &frame_id, string const &child_frame_id, skeleton_markers::Skeleton &skeleton)
      {
        ...
        transform.setRotation(tf::Quaternion(qz,qx, qy, qw)*tf::createQuaternionFromYaw(PI));
...
int main(int argc, char** argv)
{
  ...
  np.param<bool>("visualization", visualize, false); 
  np.param("frequency", frequency, 30);     
